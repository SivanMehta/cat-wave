...
void main(void) {
  ...
  vec2 click_position = (last_click.xy / resolution.xy);
  // the color at the given position
  vec4 color = texture2D(u_texture, position);
  // all red color for debugging
  // vec4 color = vec4(1.0, 0.0, 0.0, 1.0);

  // determine how much to apply the mixer by the distance to click_position;
  float value = distance(position, click_position) * 10.0 - time * 4.0;
  float dampening;
  if(sin(value) < 0.0) {
    dampening = 1.0;
  } else {
